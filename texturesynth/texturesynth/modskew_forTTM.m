function [chm, snrk,snrk_w,problemFlag] = modskew_forTTM(ch,mask,sk)
...
if any(size(mask)~=size(ch))
    disp(sprintf('Mask size and image size must be the same! Mask = %d x %d, Image = %d x %d',size(mask,1),size(mask,2),size(ch,1),size(ch,2)));
end

ch2 = ch(mask);
[chout, snrk,snrk_w,problemFlag] = modskew(ch2, sk, p);
if ~problemFlag
    chm = ch;
    chm(mask) = chout;
else
    chm = ch;
    disp('NaNs in the result of modskew!');    
end
...
% chm=chm*sqrt(m(2)/var_forTTM(chm,mask));		% adjust the variance
